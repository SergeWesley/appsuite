...
import { WatchingSession, WatchingSessionFormData } from '@/types/watching-session';
import { Database } from '@/types/supabase';
import { supabase } from '@/lib/supabase';
import { useAuth } from './useAuth';

type SessionRow = Database['public']['Tables']['watching_sessions']['Row'];
type SessionInsert = Database['public']['Tables']['watching_sessions']['Insert'];
type SessionUpdate = Database['public']['Tables']['watching_sessions']['Update'];
...
export function useWatchingSessions(onMediaDataChanged?: () => void) {
  ...
  const [activeSessions, setActiveSessions] = useState<Map<string, WatchingSession>>(new Map());
  ...
  const startSession = useCallback(async (mediaId: string, episodeWatched?: number, seasonWatched?: number): Promise<WatchingSession | null> => {
    if (!user) {
    ...
    try {
      ...
      setSessions(prev => {
        ...
        const filtered = prev.filter(s => !(s.mediaId === mediaId && s.isActive));
  ...
  const stopSession = useCallback(async (mediaId: string, sessionData?: Partial<WatchingSessionFormData>): Promise<WatchingSession | null> => {
    if (!user) {
      setError('Utilisateur non connecté');
      return null;
    }
    ...
    try {
      ...
      const updateData: SessionUpdate = {
        ...
        episode_watched: sessionData?.episodeWatched || activeSession.episodeWatched || null,
        season_watched: sessionData?.seasonWatched || activeSession.seasonWatched || null,
      };
  ...
  const getSessionsForMedia = useCallback((mediaId: string): WatchingSession[] => {
    return sessions.filter(session => session.mediaId === mediaId);
  }, [sessions]);

  // Obtenir les statistiques de visionnage pour un média
  const getMediaStats = useCallback((mediaId: string): MediaWatchingStats => {
    const mediaSessions = getSessionsForMedia(mediaId);
    const completedSessions = mediaSessions.filter(session => !session.isActive);
    
    const totalSessions = completedSessions.length;
    const totalWatchingTime = completedSessions.reduce((acc, session) => acc + (session.duration || 0), 0);
    const averageSessionTime = totalSessions > 0 ? totalWatchingTime / totalSessions : 0;
  ...
  const isSessionActive = useCallback((mediaId: string): boolean => {
    return activeSessions.has(mediaId);
  }, [activeSessions]);

  // Obtenir la session active pour un média
  const getActiveSession = useCallback((mediaId: string): WatchingSession | undefined => {
  ...
  const getCurrentSessionTime = useCallback((mediaId: string): number => {
    const activeSession = activeSessions.get(mediaId);
  ...
  const getFormattedCurrentTime = useCallback((mediaId: string): string => {
