...
function mapFormDataToInsert(formData: MediaFormData, userId: string): MediaInsert {
  ...
  return {
    ...
    date_added: now,
    date_started: formData.status === 'watching' || formData.status === 'completed' ? now : null,
...
export function useMedias() {
  ...
  const updateMedia = async (id: string, updates: Partial<MediaFormData>): Promise<boolean> => {
    if (!user) {
      setError('Utilisateur non connecté');
      return false;
    }
  ...
  const getStats = () => {
    ...
    const watching = medias.filter(media => media.status === 'watching').length;
    const completed = medias.filter(media => media.status === 'completed').length;
    const toWatch = medias.filter(media => media.status === 'towatch').length;
    const wishlist = medias.filter(media => media.status === 'wishlist').length;
    ...
    const documentaries = medias.filter(media => media.type === 'documentary').length;
    const shorts = medias.filter(media => media.type === 'short').length;
    
    const averageRating = medias
