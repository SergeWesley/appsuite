...
export function useKafka() {
  ...
  const sendEventToAPI = useCallback(
    async (
      eventType: "connection" | "disconnection",
      userData: UserConnectionEventData,
    ) => {
      try {
        const sessionId =
          userData.sessionId || sessionIdRef.current || "unknown";
        ...
        if (!response.ok) {
          ...
          throw new Error(
            `Erreur API: ${response.status} - ${errorData.error || "Erreur inconnue"}`,
          );
        }
        ...
        console.error(
          `Erreur lors de l'envoi de l'événement ${eventType}:`,
          error,
        );
  ...
  const sendConnectionEvent = useCallback(
    async (userData: UserConnectionEventData) => {
      try {
        await sendEventToAPI("connection", userData);
      } catch (error) {
        console.error(
          "Erreur lors de l'envoi de l'événement de connexion:",
          error,
        );
  ...
  const sendDisconnectionEvent = useCallback(
    async (userData: UserConnectionEventData) => {
      try {
        ...
        console.error(
          "Erreur lors de l'envoi de l'événement de déconnexion:",
          error,
        );
