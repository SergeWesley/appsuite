...
type WorkoutSessionUpdate =
  Database["public"]["Tables"]["workout_sessions"]["Update"];
...
function mapRowToWorkoutSession(
  row: WorkoutSessionWithExercises,
): WorkoutSession {
  return {
    id: row.id,
...
export function useWorkoutSessions() {
  ...
  const addSession = async (
    sessionData: WorkoutSessionFormData,
    ...
    try {
      ...
      if (sessionData.exercises.length > 0) {
        const exerciseInserts: WorkoutExerciseInsert[] =
  ...
  const updateSession = async (
    id: string,
    updates: Partial<WorkoutSessionFormData>,
  ): Promise<boolean> => {
    if (!user) {
      setError("Utilisateur non connecté");
      return false;
    ...
    try {
      ...
      if (updates.date !== undefined)
        sessionUpdate.date = updates.date.toISOString().split("T")[0];
      if (updates.notes !== undefined)
        sessionUpdate.notes = updates.notes || null;
      if (updates.exercises !== undefined)
        sessionUpdate.total_exercises = updates.exercises.length;
  ...
  const duplicateSession = async (
    ...
    try {
      ...
      const duplicatedSession: WorkoutSessionFormData = {
        ...
        exercises: originalSession.exercises.map(
  ...
  const getStats = (): WorkoutStats => {
    ...
    const exercisesByMuscleGroup = sessions.reduce(
      (acc, session) => {
    ...
    const sessionsThisMonth = sessions.filter(
      (session) => session.date >= oneMonthAgo,
    ).length;
