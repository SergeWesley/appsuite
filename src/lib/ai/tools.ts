...
export const getAgentTools = (supabase: SupabaseClient, userId: string) => {
  return {
    ...
    // --- MODULE NOTES : Lecture de contenu ---
    getNoteContentTool: tool({
      description:
      ...
      execute: async ({ noteId }) => {
        ...
        if (note.template_id) {
          ...
          if (template?.fields && Array.isArray(template.fields)) {
            templateFields = (template.fields as any[]).map(
          ...
          metadataSummary = Object.entries(meta)
            .map(([key, value]) => {
              if (Array.isArray(value)) {
                // C'est probablement un tableau (type "table")
                return `Tableau "${key}" : ${value.length} entrée(s) — ${JSON.stringify(value)}`;
              }
        ...
        return {
          ...
          templateFields:
            templateFields.length > 0
              ? templateFields.map((f) => `${f.name} (${f.type})`).join(", ")
              : "Aucun template",
